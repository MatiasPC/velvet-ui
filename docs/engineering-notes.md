...
| An animation-only mask still has to be neutral at rest | The `Capsule()` that masks `DSSlideToConfirm`'s pill into a circle for `.morphAndVanish` is sized to the track, so at rest its straight top and bottom edges land exactly on the knob's tangent points and the mask's antialiased edge shaves ~1pt off each — the knob measured 168×166 instead of 168×168 at @3× and stopped reading as round. `maskOutset` (`Metrics.collapseMaskOutset`, `DSSpacing.xl`) insets the capsule *outward* on every side until the collapse starts, then rides `collapseInset`'s spring back to 0 so the morph still resolves on a capsule exactly `trackHeight` square. A full-bleed mask that only bites during an animation needs to be held clear of everything it would otherwise crop while it's idle. |
