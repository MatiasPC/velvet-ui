...
| Scroll-linked motion → `scrollTransition`, not `onAppear` | `.dsScrollReveal` reacts to the scroll offset instead of playing once on appear (which is what `.dsStaggerIn` / `.dsPopIn` do). Inside `scrollTransition` you may only return `some VisualEffect` — `opacity`, `scaleEffect`, `offset`, etc. — not arbitrary view modifiers, and the branches of an `if` must return the same effect type, so drive each transform off `phase.isIdentity` / `phase.value` rather than swapping modifier chains. `phase.value` runs −1 (leading edge) → 0 (identity) → +1 (trailing edge); a symmetric `offset(y: rise * phase.value)` lifts a row the same amount entering and leaving. Reduce Motion returns the bare `content` (fully visible), never a scroll-linked fade. The modifier is inert outside a `ScrollView`, so it is safe to leave attached. |
