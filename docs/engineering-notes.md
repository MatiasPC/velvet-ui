...
- **Shake is a `GeometryEffect`, not an `.offset`.** `.dsShake(trigger:)` shakes via `DSShakeEffect` (a damped sine on an animatable count) so it moves pixels without touching layout — safe on any content, including inside stacks. `DSCodeField` still carries its own private `DSCodeFieldShake`; fold it into `DSShakeEffect` on the next `DSCodeField` restyle rather than in a drive-by. Amplitude fades with the fractional part of the count, so start and end sit at rest — drive it by incrementing inside `withAnimation(DSShake.animation)` (a curve, not a spring: a spring on the driver smears the wave).
