...
| `DSSlider` maps touches in `GeometryReader` | The knob occupies `knobSize`, so the draggable travel is `width − knobSize`, not `width`. The fraction is `(location.x − knobSize/2) / usable` (clamped 0…1) and the knob offsets by `fraction · usable` (its center never leaves the track). Continuous drags set the value with no animation so the knob tracks the finger 1:1; stepped values snap with `springSnappy`. Don't add an implicit `.animation(value:)` to the fraction or continuous drags will lag behind the thumb. |
