...
| `dsScrollReveal` goes on the items, not the `ScrollView` | `scrollTransition` measures each subview against its enclosing scroll container, so the modifier must sit on the rows/cards/cells inside the scroll — exactly like `dsStaggerIn`. Put it on the `ScrollView` (or on a non-scrolled view) and it silently renders identity: no crash, no effect. `.interactive` interpolates continuously with the offset (Today-feed feel); `.animated` would snap at a threshold. Under Reduce Motion only the opacity fade survives — scale and blur pin to identity — because opacity is not motion. |
