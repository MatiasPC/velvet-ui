...
| `DSToggle` knob geometry | The knob is a `Circle` whose diameter equals `trackHeight`, so it nests exactly into the `Capsule` ends (equal radii) and no track fill shows above or below it. `travel` is `(trackWidth − trackHeight) / 2` — no edge inset. Its `.dsShadow(.sm)` renders outside the `trackWidth × trackHeight` frame; nothing clips `switchTrack`, so keep it that way. |
